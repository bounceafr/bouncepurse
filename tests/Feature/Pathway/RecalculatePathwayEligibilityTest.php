...
test('recalculation skips players without a profile', function (): void {
    ...
    $playerWithoutProfile = User::factory()->create()->assignRole(Role::Player->value);

    $action = resolve(RecalculateAllPathwayEligibilityAction::class);
    $action->handle($config);

    expect($playerWithoutProfile->profile)->toBeNull();
});

test('recalculate pathway eligibility job dispatches action', function (): void {
    $config = PathwayConfiguration::factory()->create([
        'min_approved_games' => 3,
        'max_rank' => 10,
        'max_conduct_flags' => 5,
