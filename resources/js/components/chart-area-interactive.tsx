...
export function ChartAreaInteractive({ data }: { data: VisitorData[] }) {
  const isMobile = useIsMobile()
  const initialTimeRange = isMobile ? "7d" : "90d"
  const [timeRange, setTimeRange] = React.useState(initialTimeRange)

  const referenceDateStr = data[data.length - 1]?.date ?? new Date().toISOString()
      ...
      <CardHeader>
        ...
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            {totalVisitors.toLocaleString()} total sessions
          </span>
